...
struct fsl_usb2_platform_data {
	...
	const char *regu1_name;
	const char *regu2_name;
	int (*gpio_usb_active) (void);
	void (*gpio_usb_inactive) (void);
	int (*pwr_enable) (void);
	int (*pwr_disable) (void);
	void (*gpio_usb_xcvr_cs_enable) (int);
	void (*gpio_usb_xcvr_reset) (void);

	int delay_probe;        /* delay port probe */
	int probed;             /* probed or not */
	int fs_limit;           /* limit HS devices to FS */
