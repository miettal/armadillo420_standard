...
enum {RS485_FLAGS, RS485_DELAY_RTS_BEFORE_SEND, RS485_DELAY_RTS_AFTER_SEND};
static unsigned int rs485_uart1[3];
static unsigned int rs485_uart2[3];
static unsigned int rs485_uart3[3];
static unsigned int rs485_uart4[3];
static unsigned int rs485_uart5[3];
static unsigned int rs485_uart6[3];
static unsigned int (*rs485_uarts[6])[3] = {&rs485_uart1, &rs485_uart2,
					    &rs485_uart3, &rs485_uart4,
					    &rs485_uart5, &rs485_uart6};

static int mxcuart_rs485_get_de(struct uart_port *port)
{
	volatile unsigned int cr2;

	cr2 = readl(port->membase + MXC_UARTUCR2);

	return !(cr2 & MXC_UARTUCR2_CTS);
}

static void mxcuart_rs485_set_de(struct uart_port *port, int enable)
{
	volatile unsigned int cr2;

	cr2 = readl(port->membase + MXC_UARTUCR2);
	cr2 &= ~MXC_UARTUCR2_CTSC;
	cr2 |= MXC_UARTUCR2_SRST;
	if (enable) {
		if (port->rs485.flags & SER_RS485_RX_DURING_TX)
			cr2 |= MXC_UARTUCR2_RXEN;
		else
			cr2 &= ~MXC_UARTUCR2_RXEN;
		cr2 &= ~MXC_UARTUCR2_CTS;
	} else {
		cr2 |= (MXC_UARTUCR2_CTS | MXC_UARTUCR2_RXEN);
	}

	writel(cr2, port->membase + MXC_UARTUCR2);
}

static void mxcuart_rs485_tx_complete(struct work_struct *work)
{
	uart_mxc_port *umxc;
	unsigned long flags;

	spin_lock_irqsave(&umxc->port.lock, flags);
	umxc = container_of(work, uart_mxc_port, rs485_after_send.work);
	mxcuart_rs485_set_de(&umxc->port, umxc->port.rs485.flags &
			     SER_RS485_RTS_AFTER_SEND);
	spin_unlock_irqrestore(&umxc->port.lock, flags);
}

static void mxcuart_rs485_driver_disable(uart_mxc_port *umxc)
{
	unsigned long msecs;
	int ret;

	if (umxc->port.rs485.delay_rts_after_send > 0) {
		msecs = msecs_to_jiffies(umxc->port.rs485.delay_rts_after_send);
		ret = schedule_delayed_work(&umxc->rs485_after_send, msecs);
		if (!ret) {
			cancel_delayed_work(&umxc->rs485_after_send);
			schedule_delayed_work(&umxc->rs485_after_send, msecs);
		}
	} else {
		mxcuart_rs485_set_de(&umxc->port,
				     umxc->port.rs485.flags &
				     SER_RS485_RTS_AFTER_SEND);
	}
}
...
static void mxcuart_start_tx(struct uart_port *port)
{
	...
	if (port->rs485.flags & SER_RS485_ENABLED) {
		de = mxcuart_rs485_get_de(&umxc->port);

		/* check if already start */
		if (!(de ^ !!(port->rs485.flags & SER_RS485_RTS_ON_SEND)))
			cancel_delayed_work(&umxc->rs485_after_send);

		/* check if tx is stopped */
		if (!(de ^ !!(port->rs485.flags & SER_RS485_RTS_AFTER_SEND))) {
			mxcuart_rs485_set_de(port, port->rs485.flags &
					     SER_RS485_RTS_ON_SEND);
			udelay(1); /* Driver Output Enable Time */
...
static void mxcuart_tx_chars(uart_mxc_port * umxc)
{
	...
	if (umxc->port.rs485.flags & SER_RS485_ENABLED)
		cancel_delayed_work(&umxc->rs485_after_send);
	...
	do {
		...
		if (sr2 & MXC_UARTUSR2_TXDC) {
			...
			if (umxc->port.rs485.flags & SER_RS485_ENABLED) {
				...
				cr &= ~MXC_UARTUCR4_TCEN;
				writel(cr, umxc->port.membase + MXC_UARTUCR4);
				mxcuart_rs485_driver_disable(umxc);
			}
		}
...
static irqreturn_t mxcuart_dsr_int(int irq, void *dev_id)
{
	uart_mxc_port *umxc = dev_id;

	if (irq != gpio_to_irq(umxc->gpio_dsr))
		return IRQ_NONE;
...
static unsigned int mxcuart_get_mctrl(struct uart_port *port)
{
	...
	if (umxc->use_gpio_for_dsr && gpio_is_valid(umxc->gpio_dsr)) {
	    if (gpio_get_value(umxc->gpio_dsr) == 0)
		result |= TIOCM_DSR;
	}
...
static int mxcuart_startup(struct uart_port *port)
{
	...
	/* Request gpio for DSR if gpio is valid. */
	if (umxc->use_gpio_for_dsr && gpio_is_valid(umxc->gpio_dsr)) {
		int irq;

		retval = gpio_request(umxc->gpio_dsr, "UART2_DSR");
		if (retval != 0)
			return retval;
		gpio_direction_input(umxc->gpio_dsr);

		irq = gpio_to_irq(umxc->gpio_dsr);
		if (irq < 0)
			return irq;

		retval = request_irq(irq, mxcuart_dsr_int,
				     IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "mxcintuart-gpio",
				     umxc);
		if (retval != 0)
			return retval;
	}
	...
	if (umxc->port.rs485.flags & SER_RS485_ENABLED) {
		...
		spin_lock_irqsave(&umxc->port.lock, flags);
		if (!mxcuart_rs485_get_de(&umxc->port))
			cr2 |= MXC_UARTUCR2_CTS;
	...
	/*
	 * Enable the UART and the Receive interrupts
	 */
	if (umxc->ir_mode == IRDA) {
...
static void mxcuart_shutdown(struct uart_port *port)
{
	...
	/* Free gpio for dsr */
	if (umxc->use_gpio_for_dsr && gpio_is_valid(umxc->gpio_dsr)) {
		free_irq(gpio_to_irq(umxc->gpio_dsr), umxc);
		gpio_free(umxc->gpio_dsr);
	}
				...
				struct ktermios *termios, struct ktermios *old)
{
	...
	if (port->rs485.flags & SER_RS485_ENABLED) {
		cr2 |= (MXC_UARTUCR2_IRTS | MXC_UARTUCR2_SRST);
		if (!(umxc->port.rs485.flags & SER_RS485_RTS_AFTER_SEND))
			cr2 |= MXC_UARTUCR2_CTS;
	} else {
 ...
 *
 * @param   port   the port structure for the UART passed in by the core driver
 * @param   rs485  the desired rs485 settings
 */
				...
				struct serial_rs485 *rs485)
{
	...
	if (rs485->flags & SER_RS485_ENABLED) {
		if (!umxc->rs485_duplex || (umxc->ir_mode == IRDA))
			return -EOPNOTSUPP;
		...
		if (rs485->flags & SER_RS485_RX_DURING_TX)
			umxc->rs485_duplex(RS485_DUPLEX_FULL);
		else
			umxc->rs485_duplex(RS485_DUPLEX_HALF);
	}
...
static struct uart_ops mxc_ops = {
	...
	.rs485_config = mxcuart_rs485_config,
};
				  ...
				  u_int count)
{
	...
	if (mxc_ports[co->index]->use_gpio_for_dsr &&
	    gpio_is_valid(mxc_ports[co->index]->gpio_dsr))
		disable_irq(gpio_to_irq(mxc_ports[co->index]->gpio_dsr));
	...
	if (mxc_ports[co->index]->use_gpio_for_dsr &&
	    gpio_is_valid(mxc_ports[co->index]->gpio_dsr))
		enable_irq(gpio_to_irq(mxc_ports[co->index]->gpio_dsr));
}
...
static int mxcuart_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (!device_may_wakeup(umxc->tty_dev)) {
		if (umxc->regu)
			regulator_disable(umxc->regu);
		gpio_uart_inactive(umxc->port.line, umxc->ir_mode);
	}
...
struct mxcuart_match {
	struct uart_port *port;
	struct uart_driver *driver;
};

static int mxcuart_match_port(struct device *dev, void *data)
{
	struct mxcuart_match *match = data;
...
static struct device *mxcuart_get_tty_dev(struct uart_port *port)
{
	struct mxcuart_match match;

	match.port = port;
	match.driver = &mxc_reg;

	return device_find_child(port->dev, &match, mxcuart_match_port);
}
...
static int mxcuart_probe(struct platform_device *pdev)
{
	...
	if (mxc_ports[id]->enabled == 1) {
		...
		/* propagate wakeup state to tty dev */
		mxc_ports[id]->tty_dev = mxcuart_get_tty_dev(&mxc_ports[id]->port);
		device_set_wakeup_enable(mxc_ports[id]->tty_dev, !!mxc_ports[id]->wakeup);

		if (mxc_ports[id]->regu_name) {
			mxc_ports[id]->regu = regulator_get(&pdev->dev, mxc_ports[id]->regu_name);
			if (IS_ERR(mxc_ports[id]->regu))
				return PTR_ERR(mxc_ports[id]->regu);
			regulator_enable(mxc_ports[id]->regu);
		}

		INIT_DELAYED_WORK(&mxc_ports[id]->rs485_after_send,
				  mxcuart_rs485_tx_complete);

		port = &mxc_ports[id]->port;
		if ((*rs485_uarts[id])[RS485_FLAGS] & SER_RS485_ENABLED) {
			port->rs485.flags = (*rs485_uarts[id])[RS485_FLAGS];
			port->rs485.delay_rts_before_send =
				(*rs485_uarts[id])[RS485_DELAY_RTS_BEFORE_SEND];
			port->rs485.delay_rts_after_send =
				(*rs485_uarts[id])[RS485_DELAY_RTS_AFTER_SEND];
		}
		if (port->rs485.flags & SER_RS485_ENABLED) {
			ret = mxcuart_rs485_config(port, &port->rs485);
			if (ret < 0)
				return -1;
			writel(MXC_UARTUCR1_UARTEN, port->membase + MXC_UARTUCR1);
		}
