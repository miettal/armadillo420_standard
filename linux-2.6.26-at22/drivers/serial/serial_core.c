...
static int uart_get_rs485_config(struct uart_state *state,
...
static int uart_set_rs485_config(struct uart_state *state,
				 struct serial_rs485 __user *rs485_user)
{
	...
	spin_lock_irqsave(&port->lock, flags);
	ret = port->ops->rs485_config(port, &rs485);
	spin_unlock_irqrestore(&port->lock, flags);
	if (ret)
		return ret;
	   ...
	   unsigned long arg)
{
	...
	case TIOCGRS485:
		ret = uart_get_rs485_config(state, uarg);
		break;

	case TIOCSRS485:
		ret = uart_set_rs485_config(state, uarg);
		break;
	default: {
		struct uart_port *port = state->port;
		if (port->ops->ioctl)
