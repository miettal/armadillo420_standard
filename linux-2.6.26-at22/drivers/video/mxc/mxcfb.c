...
static char *videomode = NULL;
static int videomodeidx = -1;
static int disp_iface = 0;
static int bpp = CONFIG_FB_MXC_DEFAULT_BPP;
static int enable = CONFIG_FB_MXC_DEFAULT_STATE;
...
static int mxcfb_set_par(struct fb_info *fbi)
{
	...
	if (mxc_fbi->ipu_ch == MEM_SDC_BG) {
		...
		if (disp_iface & MXCFB_DISP_CLK_INVERT)
			sig_cfg.clk_pol = true;
		if (disp_iface & MXCFB_DISP_DATA_INVERT)
			sig_cfg.data_pol = true;
		if (disp_iface & MXCFB_DISP_OE_ACT_HIGH)
			sig_cfg.enable_pol = true;
		if (disp_iface & MXCFB_DISP_CLK_IDLE_EN)
			sig_cfg.clkidle_en = true;
		if (disp_iface & MXCFB_DISP_SHARP_MODE)
		...
		if (ipu_sdc_init_panel(mode,
				       ...
				       (disp_iface & MXCFB_DISP_SWAP_RGB) ?
	...
	ipu_sdc_set_window_pos(mxc_fbi->ipu_ch,
			       fbi->var.hsync_len,
			       fbi->var.vsync_len);
	...
	case ENCODER_SET_NORM:
		{
			...
			fbi->var = var;
			videomode = smode;
			disp_iface = mxcfb_disp_iface_from_mode(smode);
...
static int mxcfb_blank(int blank, struct fb_info *info)
{
	...
	case FB_BLANK_NORMAL:
		...
		if (videomode) {
			...
			if ((strcmp(videomode, MODE_VGA) == 0)
			    || (strcmp(videomode, MODE_NTSC) == 0)
			    || (strcmp(videomode, MODE_PAL) == 0))
	...
	case FB_BLANK_UNBLANK:
		...
		if (videomode) {
			...
			else if (strcmp(videomode, MODE_NTSC) == 0)
				mode = VIDEO_ENCODER_NTSC;
			else if (strcmp(videomode, MODE_PAL) == 0)
...
mxcfb_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	if (base == mxc_fbi->pan_offset)
		return 0;

	spin_lock_irqsave(&mxc_fbi->fb_lock, lock_flags);
	mxc_fbi->pan_offset = base;
	mxc_fbi->pan_display = 1;
...
static irqreturn_t mxcfb_irq_handler(int irq, void *dev_id)
{
	...
	spin_lock_irqsave(&mxc_fbi->fb_lock, flags);

	/* flip buffer if pan has been requested */
	if (mxc_fbi->pan_display) {
		ret = ipu_update_channel_buffer(mxc_fbi->ipu_ch,
				IPU_INPUT_BUFFER,
				!mxc_fbi->cur_ipu_buf,
				fbi->fix.smem_start + mxc_fbi->pan_offset);
		if (ret == 0) {
			...
			dev_err(fbi->device,
				"Error updating SDC buf %d base address\n",
				!mxc_fbi->cur_ipu_buf);
		}
		mxc_fbi->pan_display = 0;
	}

	spin_unlock_irqrestore(&mxc_fbi->fb_lock, flags);
...
static int mxcfb_check_pixel_clock(struct platform_device *pdev, u32 timing)
{
	struct clk *clk_info;
	u32 hsp_clk;
	u32 pix_clk;
	...
	clk_info = clk_get(&pdev->dev, "ipu_clk");
	hsp_clk = clk_get_rate(clk_info);

	pix_clk = (1000000000 / timing) * 1000;
	rate = hsp_clk / (pix_clk/10);
	dev_dbg(&pdev->dev, "hsp_clk: %10dHz\n", hsp_clk);
	dev_dbg(&pdev->dev, "pix_clk: %10dHz\n", pix_clk);
	dev_dbg(&pdev->dev, "rate: %d.%d\n", rate/10, rate%10);
	if (rate < 40) {
		dev_err(&pdev->dev,
			"not support pixel-clock: %dHz\n", pix_clk);
		return -1;
	}
	return 0;
}
...
/*
 * Parse user specified options (`video=mxcfb:')
 * example:
 * 	video=mxcfb:CRT-SVGA,16bpp,enable
 */
static int mxcfb_setup(struct platform_device *pdev)
{
	int ret;
	int i;
	...
	while ((arg = strsep(&args, ",")) != NULL) {
		if (!*arg) {
			...
			enable = 0;
		} else if (!strcmp(arg, "16bpp")) {
			bpp = 16;
		} else if (!strcmp(arg, "24bpp")) {
			bpp = 24;
		} else if (!strcmp(arg, "32bpp")) {
			bpp = 32;
		} else {
			for (i=0; i<mxcfb_modedb_sz; i++) {
				if (!strcmp(arg, mxcfb_modedb[i].name)) {
					videomode = arg;
					continue;
				}
			}
	...
	if (!videomode)
		videomode = CONFIG_FB_MXC_DEFAULT_VIDEOMODE;

	for (i=0; i<mxcfb_modedb_sz; i++) {
		if (strcmp(videomode, mxcfb_modedb[i].name) == 0) {
			videomodeidx = i;
			break;
		}
	}
	if (videomodeidx == -1) {
		dev_err(&pdev->dev, "not support videomode: %s\n", videomode);
		return -EINVAL;
	}
	ret = mxcfb_check_pixel_clock(pdev,
				      mxcfb_modedb[videomodeidx].pixclock);
	if (ret)
		return -EINVAL;

	disp_iface = mxcfb_disp_iface_from_mode(videomode);
	...
	case 32:
		break;
	default:
		dev_err(&pdev->dev, "not support bpp: %d\n", bpp);
		return -EINVAL;
	}

	dev_dbg(&pdev->dev, "videomode: %s\n", videomode);
	dev_dbg(&pdev->dev, "      bpp: %d\n", bpp);
	dev_dbg(&pdev->dev, "   enable: %d\n", enable);
...
static int mxcfb_probe(struct platform_device *pdev)
{
	...
	if (!fb_find_mode(&fbi->var, fbi, videomode, mxcfb_modedb,
			  mxcfb_modedb_sz, NULL, bpp)) {
		ret = -EBUSY;
		goto err2;
	}
...
static int mxcfb_remove(struct platform_device *pdev)
{
	...
	if (!cpu_is_mx31() && !cpu_is_mx32())
		ipu_free_irq(IPU_IRQ_SDC_DISP3_VSYNC, drv_data);
...
static struct platform_driver mxcfb_driver = {
	...
	.probe		= mxcfb_probe,
	.remove		= __devexit_p(mxcfb_remove),
 ...
 * @return      Error code indicating success or failure
 */
static int __init mxcfb_init(void)
{
	return platform_driver_register(&mxcfb_driver);
}

static void __exit mxcfb_exit(void)
{
...
module_param(videomode, charp, 0);
