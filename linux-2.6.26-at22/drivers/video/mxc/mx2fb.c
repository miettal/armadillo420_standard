 ...
 * Copyright 2010 Atmark Techno, Inc. All Rights Reserved.
 */
...
extern void board_power_lcd(int on, const char *mode);
...
/* Internal functions */
static int __devinit _init_fbinfo(struct fb_info *info,
				  struct platform_device *pdev);
static int __devinit _install_fb(struct fb_info *info,
				 struct platform_device *pdev);
...
static int _event_notifier_call(struct notifier_block *self,
...
static int __devinit mx2fb_probe(struct platform_device *pdev);
...
static struct notifier_block mx2fb_event_notifier = {
	.notifier_call = _event_notifier_call,
};
...
static int _wait_for_pan_complete(struct mx2fb_info *mx2fbi)
{
	long timeout =  msecs_to_jiffies(100);

	while(!mx2fbi->flip_done) {
		timeout = wait_event_interruptible_timeout(mx2fbi->flip_wq,
							   mx2fbi->flip_done,
							   timeout);
		if (mx2fbi->flip_done)
			return 0;
		...
		if (timeout == 0) {
			pr_err("%s: wait for pan complete timeout %s\n",
			       __func__, mx2fbi->id);
			return -ETIMEDOUT;
		}
...
static int mx2fb_pan_display(struct fb_var_screeninfo *var,
			     struct fb_info *info)
{
	struct mx2fb_info *mx2fbi = info->par;
	unsigned long base;
	...
	/* Set Screen Start Address Register if changed */
	if (base != readl(LCDC_REG(LCDC_LSSAR))) {
		enable_irq(MXC_INT_LCDC);

		mx2fbi->flip_done = 0;
		__raw_writel(base, LCDC_REG(LCDC_LSSAR));

		ret = _wait_for_pan_complete(mx2fbi);

		disable_irq(MXC_INT_LCDC);
	}
				 ...
				 struct platform_device *pdev)
{
	...
	disp_iface = mxcfb_disp_iface_from_mode(fb_mode);
	...
	init_waitqueue_head(&mx2fbi->flip_wq);
...
static void _update_lcdc(struct fb_info *info)
{
	...
	pcd = (perclk3 / (PICOS2KHZ(var->pixclock) * 1000UL));
	if (--pcd > 0x3F)
		pcd = 0x3F;

	/* Panel configuration register */
	pcr = 0xFA008B80 | pcd;
	pcr |= (var->sync & FB_SYNC_HOR_HIGH_ACT) ? 0 : 0x00800000;
	pcr |= (var->sync & FB_SYNC_VERT_HIGH_ACT) ? 0 : 0x00400000;
	pcr |= (disp_iface & MXCFB_DISP_CLK_INVERT) ? 0 : 0x00200000;
	pcr |= (disp_iface & MXCFB_DISP_DATA_INVERT) ? 0x01000000 : 0;
	pcr |= (disp_iface & MXCFB_DISP_SHARP_MODE) ? 0x00000040 : 0;
	pcr |= (disp_iface & MXCFB_DISP_OE_ACT_HIGH) ? 0 : 0x00100000;
				...
				unsigned long action, void *data)
{
	...
	struct fb_info *fbi = (struct fb_info *)event->info;
	struct mx2fb_info *mx2fbi = (struct mx2fb_info *)fbi->par;

	switch (action) {
	case FB_EVENT_MXC_EOF: /* fall through */
	case MX2FB_INT_GW_EOF:
		mx2fbi->flip_done = 1;
		wake_up_interruptible(&mx2fbi->flip_wq);
		break;
	default:
		break;
	}
...
static void _request_irq(void)
{
	...
	if (request_irq(MXC_INT_LCDC, mx2fb_isr, IRQF_DISABLED, "LCDC", 0))
		pr_err("Request LCDC IRQ failed.\n");
	else {
		disable_irq(MXC_INT_LCDC);
...
static int __devinit mx2fb_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < sizeof(mx2fb_info) / sizeof(struct fb_info); i++) {
		struct fb_info *info = &mx2fb_info[i];
		struct mx2fb_info *mx2fbi = (struct mx2fb_info *)info->par;
		if (mx2fbi->type == MX2FB_TYPE_GW) {
			_enable_graphic_window(info);
			_disable_graphic_window(info);
 ...
 * Example: video=mxcfb:bpp=16,Sharp-QVGA
 */
