...
static void sdhci_send_command(struct sdhci_host *host, struct mmc_command *cmd)
{
	...
	if (cmd->data != NULL) {
		...
		if (cmd->data->blocks > 1) {
			...
			if (cpu_is_mx25() || cmd->opcode == 0x35) {
...
static void sdhci_set_clock(struct sdhci_host *host, unsigned int clock)
{
	...
	before = readl(host->ioaddr + SDHCI_CLOCK_CONTROL);
	clk = before & ~SDHCI_CLOCK_MASK;
	...
	if (readl(host->ioaddr + SDHCI_DLL_STATUS) & DLL_STS_SLV_LOCK) {
		/* reset DLL CTRL */
...
static void sdhci_set_power(struct sdhci_host *host, unsigned short power)
{
	...
	if (host->regulator_mmc) {
		if (power == (unsigned short)-1) {
			gpio_sdhc_inactive(host->chip->pdev->id);
			...
			if (regulator_is_enabled(host->regulator_mmc) == 0) {
				if (regulator_enable(host->regulator_mmc) == 0)
					DBG("mmc power on\n");
			}
			gpio_sdhc_active(host->chip->pdev->id);
		}
	}
...
static int sdhci_clock_start(struct mmc_host *mmc)
{
...
static int sdhci_clock_stop(struct mmc_host *mmc)
{
	struct sdhci_host *host = mmc_priv(mmc);
...
static const struct mmc_host_ops sdhci_ops = {
	...
	.clk_start = sdhci_clock_start,
	.clk_stop = sdhci_clock_stop,
};
...
static void sdhci_data_irq(struct sdhci_host *host, u32 intmask)
{
	...
	if (cpu_is_mx25() && host->data->error)
		intsave &= ~(SDHCI_INT_DATA_CRC | SDHCI_INT_DATA_END_BIT);
				      ...
				      *pdev, int slot)
{
	 ...
	 * Maximum block count.
	 */
#if defined(CONFIG_MMC_IMX_ESDHCI_FORCE_SINGLE_BLOCK_TRANSFER)
	mmc->max_blk_count = 1;
#else
	mmc->max_blk_count = 65535;
#endif /* defined(CONFIG_MMC_IMX_ESDHCI_FORCE_SINGLE_BLOCK_TRANSFER) */
