...
static struct nfc_oob_free_table free_table_8bitbus = {
	.seg1_start = 0,
	.seg1_end = 4,
	.seg2_start = 11,
	.seg2_end = 15,
...
static struct nfc_oob_free_table free_table_16bitbus = {
	.seg1_start = 0,
	.seg1_end = 5,
	.seg2_start = 12,
	.seg2_end = 15,
...
struct mxc_mtd_s {
	...
	struct device *dev;
	struct nfc_oob_free_table *oob_table;
	char oob_fake[NAND_MAX_OOBSIZE];
};
 ...
 */
static struct nand_ecclayout fake_nand_oob_16 = {
	.eccbytes = 5,
	.eccpos = {10, 11, 12, 13, 14},
	.oobfree = {{2, 8}},
};

static struct nand_ecclayout fake_nand_oob_64 = {
	.eccbytes = 20,
	.eccpos = {
...
/**
 * Copy OOB data of the specified 512k page from the fake OOB buffer to the
 * NFC's SRAM.
 */
static void fakeoob_to_nfcoob(int buf_nr)
{
	struct nfc_oob_free_table *t = mxc_nand_data->oob_table;
	char *fake_oob = mxc_nand_data->oob_fake;
	volatile u16 *nfc_oob = SPARE_AREA0;
	char buf[NAND_MAX_OOBSIZE];
	int i, j, p = 0;

	/* copy free data */
	for (i = 0; i < buf_nr; i++) {
		for (j = t->seg1_start; j <= t->seg1_end; j++)
			buf[j + i*16] = fake_oob[p++];
		for (j = t->seg2_start; j <= t->seg2_end; j++)
			buf[j + i*16] = fake_oob[p++];
	}

	/* dont need to copy ecc/bi data */

	for (i = 0; i < 8*buf_nr; i++)
		nfc_oob[i] = ((u16 *)buf)[i];
}

/**
 * Copy OOB data of the specified 512k page from the NFC's SRAM to the fake
 * OOB buffer.
 */
static void nfcoob_to_fakeoob(int buf_nr)
{
	struct nfc_oob_free_table *t = mxc_nand_data->oob_table;
	char *fake_oob = mxc_nand_data->oob_fake;
	volatile u16 *nfc_oob = SPARE_AREA0;
	...
	for (i = 0; i < buf_nr*8; i++)
		((u16 *)buf)[i] = nfc_oob[i];

	/* copy free data */
	for (i = 0; i < buf_nr; i++) {
		for (j = t->seg1_start; j <= t->seg1_end; j++)
			fake_oob[p++] = buf[j + i*16];
		for (j = t->seg2_start; j <= t->seg2_end; j++)
			fake_oob[p++] = buf[j + i*16];
	}

	/* jffs2 (2.6.18) expects all data to be 0xff on erased blocks, so we
	 * dont copy the ecc/bi data */
	memset(fake_oob + 10*buf_nr, 0xff, 6*buf_nr);
}
...
static u_char mxc_nand_read_byte(struct mtd_info *mtd)
{
	...
	u16 col, rdWord;
	volatile u16 *mainBuf = MAIN_AREA0;
	volatile u16 *spareBuf = (u16 *)mxc_nand_data->oob_fake;
			       ...
			       const u_char * buf, int len)
{
	...
	while (n) {
		...
		if (col < mtd->writesize)
			...
			p = (volatile u32 *)((ulong) (mxc_nand_data->oob_fake) -
					     mtd->writesize + (col & ~3));
...
static ssize_t
mxc_low_erase_entry(struct device *dev, struct device_attribute *attr,
...
static struct device_attribute dev_attr_low_erase = \
	__ATTR(low-erase, S_IWUSR, NULL, mxc_low_erase_entry);
...
static int __init mxcnd_probe(struct platform_device *pdev)
{
	...
	if (hardware_ecc) {
		...
		this->ecc.size = 512;
		this->ecc.bytes = 3;
		this->ecc.layout = &fake_nand_oob_64;
		NFC_CONFIG1 |= NFC_ECC_EN;
	} else {
	...
	if (flash->width == 2) {
		...
		mxc_nand_data->oob_table = &free_table_16bitbus;
	} else {
		mxc_nand_data->oob_table = &free_table_8bitbus;
	}
	...
	if (mtd->writesize == NAND_PAGESIZE_2KB) {
		if (!((NFMS >> NFMS_BIT) & 0x01)) {
			DEBUG(MTD_DEBUG_LEVEL3, "Overriding NFMS bit\n");
			NFMS |= (1 << NFMS_BIT);
		}
	} else {
		if ((NFMS >> NFMS_BIT) & 0x01) {
	...
	is2k_Pagesize = (mtd->writesize == NAND_PAGESIZE_2KB);

	if (nand_scan_tail(mtd)) {
		DEBUG(MTD_DEBUG_LEVEL0,
		      "MXC_ND: Failed to finish NAND scan.\n");
		err = -ENXIO;
		goto out_1;
	}

	if (hardware_ecc && (mtd->writesize != NAND_PAGESIZE_2KB))
		this->ecc.layout = &fake_nand_oob_16;
	...
	err = device_create_file(&pdev->dev, &dev_attr_low_erase);
	if (err)
		printk(KERN_ERR "%s: failed to create sysfs entry\n",
		       __FUNCTION__);
...
static int __exit mxcnd_remove(struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &dev_attr_low_erase);
...
static int __init mxc_nd_init(void)
{
	...
	return platform_driver_register(&mxcnd_driver);
}
