...
int gpio_export(unsigned gpio, bool direction_may_change)
{
	...
	if (status == 0) {
		status = gpio_sysfs_create_device(gpio, direction_may_change,
						  "gpio%d", gpio);

#if defined(CONFIG_GPIO_SYSFS_PRIVATE_NAMING)
		if (status == 0) {
			if (strncmp("sysfs", desc->label, strlen("sysfs")) != 0 &&
			    strncmp("?", desc->label, strlen("?")) != 0)
				status = gpio_sysfs_create_device(gpio,
								  direction_may_change,
								  desc->label);
		}
#endif
