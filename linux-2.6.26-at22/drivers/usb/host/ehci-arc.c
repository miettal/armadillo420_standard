...
static ssize_t delayed_probe_store(struct device *dev,
				   struct device_attribute *attr, const char *buf, size_t count);
static DEVICE_ATTR(delayed_probe, 0200, NULL, delayed_probe_store);
...
static void arc_reset_status(struct usb_hcd *hcd, u32 *status)
{
...
static int ehci_fsl_drv_probe(struct platform_device *pdev)
{
	...
	if (pdata->delay_probe) {
		ret = device_create_file(dev, &dev_attr_delayed_probe);
		if (ret < 0) {
			printk(KERN_ERR "ehci: sysfs_create_file failed: %d\n",
				ret);
			return ret;
		}
		return 0;
...
static int ehci_fsl_drv_remove(struct platform_device *pdev)
{
	...
	if (pdata->delay_probe) {
		device_remove_file(&pdev->dev, &dev_attr_delayed_probe);
		if (!pdata->probed)
			return 0;
	}
...
static void ehci_fsl_drv_shutdown(struct platform_device *pdev)
{
	...
	if (pdata->delay_probe && !pdata->probed)
		return;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d", &val) != 1 || val != 1) {
		printk(KERN_ERR "delayed_probe: invalid value\n");
		return -EINVAL;
	}

	if (!pdata->probed) {
		usb_hcd_fsl_probe(&ehci_fsl_hc_driver, pdev);
		pdata->probed = 1;
	}
