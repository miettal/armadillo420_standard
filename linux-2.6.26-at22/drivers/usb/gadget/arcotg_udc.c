...
/* Driver probe functions */
  ...
  * @return Returns zero on success , or a negative error code
  */
static int fsl_udc_probe_full(struct platform_device *pdev)
{
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!arcotg_udc_probed) {
		fsl_udc_probe_full(pdev);
		arcotg_udc_probed = 1;
	}
...
static int fsl_udc_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(&pdev->dev, &dev_attr_delayed_probe);
	if (ret < 0) {
