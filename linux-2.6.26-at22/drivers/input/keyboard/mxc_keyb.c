...
static int mxc_kpp_scan_matrix(void)
{
	...
	for (col = 0; col < kpp_dev.kpp_cols; col++) {	/* Col */
		...
		reg_val = __raw_readw(KPDR);
		reg_val &= ~(1 << (8 + col + kpp_dev.col_first));
		__raw_writew(reg_val, KPDR);
		...
		for (row = 0; row < kpp_dev.kpp_rows; row++) {	/* sample row */
			if (TEST_BIT(reg_val, row + kpp_dev.row_first) == 0) {
				...
				for (col = 0; col < kpp_dev.kpp_cols; col++) {
					if ((press_scancode[row][col] != -1)) {
						...
						input_event(mxckbd_dev, EV_KEY,
							    mxckpd_keycodes
							    [scancode], 1);
#ifndef CONFIG_MACH_ARMADILLO500FX
...
static int mxc_kpp_probe(struct platform_device *pdev)
{
	...
	unsigned char rowbits, colbits;

	keypad = (struct keypad_data *)pdev->dev.platform_data;

	kpp_dev.row_first	= keypad->row_first;
	kpp_dev.row_last	= keypad->row_last;
	kpp_dev.col_first	= keypad->col_first;
	kpp_dev.col_last	= keypad->col_last;

	kpp_dev.kpp_rows = kpp_dev.row_last - kpp_dev.row_first + 1;
	kpp_dev.kpp_cols = kpp_dev.col_last - kpp_dev.col_first + 1;

	key_pad_enabled = 0;

	has_leaning_key = keypad->learning;
	mxckpd_keycodes = keypad->matrix;
	mxckpd_keycodes_size = kpp_dev.kpp_rows * kpp_dev.kpp_cols;

	if ((keypad->matrix == (void *)0) || (mxckpd_keycodes_size == 0))
		return -ENODEV;
	...
	press_scancode = kmalloc(kpp_dev.kpp_rows * sizeof(press_scancode[0]),
			GFP_KERNEL);
	release_scancode = kmalloc(kpp_dev.kpp_rows *
			sizeof(release_scancode[0]), GFP_KERNEL);
	...
	for (i = 0; i < kpp_dev.kpp_rows; i++) {
		...
		release_scancode[i] = kmalloc(kpp_dev.kpp_cols *
	...
	cur_rcmap = kmalloc(kpp_dev.kpp_rows * sizeof(cur_rcmap[0]),
			GFP_KERNEL);
	prev_rcmap = kmalloc(kpp_dev.kpp_rows * sizeof(prev_rcmap[0]),
			GFP_KERNEL);
	...
	for (i = 0; i < kpp_dev.kpp_rows; i++) {
		memset(press_scancode[i], -1, sizeof(press_scancode[0][0]) *
			kpp_dev.kpp_cols);
		memset(release_scancode[i], -1, sizeof(release_scancode[0][0]) *
			kpp_dev.kpp_cols);
	}
	memset(cur_rcmap, 0, kpp_dev.kpp_rows * sizeof(cur_rcmap[0]));
	 ...
	 */
	rowbits = (1 << kpp_dev.kpp_rows) - 1;
	rowbits <<= kpp_dev.row_first;
	colbits = (1 << kpp_dev.kpp_cols) - 1;
	colbits <<= kpp_dev.col_first;	
	__raw_writew(rowbits | (colbits << 8), KPCR);
