...
int pmic_read(int reg_nr, unsigned int *reg_val)
{
	 ...
	 */
	c = i2c_smbus_read_byte_data(mc34704_client, reg_nr | 0x80);
	if (c < 0) {
...
int __devinit pmic_init_registers(void)
{
	 ...
	 */
	CHECK_ERROR(pmic_write(REG_MC34704_GENERAL2, 0x01));
...
static int __devinit is_chip_onboard(struct i2c_client *client)
{
	 ...
	 */
	val = i2c_smbus_read_byte_data(client, REG_MC34704_GENERAL2 | 0x80);
	if (val < 0)
		return -1;
...
static ssize_t mc34704_onoff_store(struct kobject *kobj,
				   ...
				   const char *buf, size_t count)
{
	...
	if (strncasecmp(buf, "off", 3) == 0) {
		printk(KERN_INFO "mc34704: The system shifts to a power off mode.\n");
		pmic_read(REG_MC34704_GENERAL2, &reg);
		reg |= 0x10;
		for (; retry; retry--) {
			ret = pmic_write(REG_MC34704_GENERAL2, reg);
			if (ret == PMIC_SUCCESS) {
				/* go to power off */
				local_irq_disable();
				local_fiq_disable();
...
static struct kobj_attribute attr_onoff = \
	__ATTR(supply, 0200, NULL, mc34704_onoff_store);
				...
				const struct i2c_device_id *id)
{
	...
	regu = regulator_get(NULL, "REG5");
	if (IS_ERR(regu))
		return PTR_ERR(regu);

	i2c_set_clientdata(client, regu);

	ret = sysfs_create_file(power_kobj, &attr_onoff.attr);
	if (ret)
		dev_err(&client->dev, "\"/sys/power/supply\" create failed\n");
...
{
	sysfs_remove_file(power_kobj, &attr_onoff.attr);
...
{
	struct regulator *regu = i2c_get_clientdata(client);
