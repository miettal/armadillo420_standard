...
enum {
	...
	FLEXCAN_ATTR_SET_RESFRAME,
	FLEXCAN_ATTR_DEL_RESFRAME,
	FLEXCAN_ATTR_SHOW_RESFRAME,
...
static struct device_attribute flexcan_dev_attr[FLEXCAN_ATTR_MAX] = {
	...
	[FLEXCAN_ATTR_RX_MAXMB] =
	    __ATTR(rx_maxmb, 0644, flexcan_show_attr, flexcan_set_attr),
	[FLEXCAN_ATTR_FIFO] =
	...
	[FLEXCAN_ATTR_SET_RESFRAME] =
	    __ATTR(set_resframe, 0644, NULL, flexcan_set_attr),
	[FLEXCAN_ATTR_DEL_RESFRAME] =
	    __ATTR(del_resframe, 0644, NULL, flexcan_set_attr),
	[FLEXCAN_ATTR_SHOW_RESFRAME] =
	    __ATTR(show_resframe, 0644, flexcan_show_attr, NULL),
#ifdef CONFIG_CAN_DEBUG_DEVICES
	[FLEXCAN_ATTR_DUMP_REG] =
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	case FLEXCAN_ATTR_STANDARD_MSG:
		return sprintf(buf, "%d\n", flexcan->std_msg);
	case FLEXCAN_ATTR_SHOW_RESFRAME:
		if (netif_running(net))
			return flexcan_mbm_show_resframe(flexcan, buf);
		else
			return 1;
#ifdef CONFIG_CAN_DEBUG_DEVICES
				...
				size_t count)
{
	...
	if (netif_running(net)) {
		switch (attr_id) {
		case FLEXCAN_ATTR_SET_RESFRAME:
			if ((ret = flexcan_mbm_set_resframe(flexcan, buf)))
				count = ret;
			break;
		case FLEXCAN_ATTR_DEL_RESFRAME:
			if ((ret = flexcan_mbm_del_resframe(flexcan, buf)))
				count = ret;
			break;
		}
		goto set_finish;
	...
	if (attr_id == FLEXCAN_ATTR_BR_CLKSRC) {
		if (sysfs_streq(buf, "bus"))
			flexcan->br_clksrc = 1;
		else if (sysfs_streq(buf, "osc"))
			flexcan->br_clksrc = 0;
	...
	case FLEXCAN_ATTR_BR_PSEG2:
		...
		break;
	case FLEXCAN_ATTR_MAXMB:
		if ((tmp > 1) && (tmp <= FLEXCAN_MAX_MB)) {
			if (flexcan->maxmb != (tmp - 1)) {
				flexcan->maxmb = tmp - 1;
				if (flexcan->maxmb <= flexcan->rx_maxmb)
					flexcan->rx_maxmb = flexcan->maxmb - 1;
			}
		}
		break;
	case FLEXCAN_ATTR_RX_MAXMB:
		if ((tmp > 0) && (tmp < (flexcan->maxmb + 1))) {
			if (flexcan->rx_maxmb != (tmp - 1))
				flexcan->rx_maxmb = tmp - 1;
		}
		break;
	case FLEXCAN_ATTR_FIFO:
...
static void flexcan_device_default(struct flexcan_device *dev)
{
	...
	dev->rx_maxmb = (FLEXCAN_MAX_MB >> 1) - 1;
	dev->xmit_mb = dev->rx_maxmb + 1;
					...
					void (*setup) (struct net_device *dev))
{
	...
	mutex_init(&flexcan->xmit_mb_mutex);
