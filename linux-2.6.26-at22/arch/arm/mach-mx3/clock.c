...
int __init mxc_clocks_init(void)
{
	...
	prcs = (__raw_readl(MXC_CCM_CCMR) & MXC_CCM_CCMR_PRCS_MASK) >>
		MXC_CCM_CCMR_PRCS_OFFSET;
	if (prcs == 0x01)
		pr_info("Clock input source is FPM(%dx1024)\n", CKIL_CLK_FREQ);
	else
		pr_info("Clock input source is %ld\n", ckih_clk.rate);

	/* update perclk clock source */
	if (!(__raw_readl(MXC_CCM_CCMR) & MXC_CCM_CCMR_PERCS)) {
		perclk_clk.parent = &usb_clk[0];
		perclk_clk.recalc = NULL;
		perclk_clk.flags  = 0;
	}
...
unsigned long __init clk_early_get_timer_rate(void)
{
	...
	usb_pll_clk.recalc(&usb_pll_clk);
	usb_clk[0].recalc(&usb_clk[0]);
	return usb_clk[0].rate;
}
