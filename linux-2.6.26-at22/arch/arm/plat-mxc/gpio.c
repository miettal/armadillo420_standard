...
static void _set_gpio_edge_ctrl(struct gpio_port *port, u32 index,
				gpio_edge_t edge)
{
	u32 icr = port->base + ((index <= 15) ? GPIO_ICR1 : GPIO_ICR2);
	u32 l, sig;

#if defined(CONFIG_ARCH_MX25)
	u32 edge_sel = port->base + GPIO_EDGE_SEL;

	l = __raw_readl(edge_sel);
	if (edge == GPIO_INT_BOTH_EDGE) {
		l |= (1 << index);
		edge = 0;
	} else
		l &= ~(1 << index);
	__raw_writel(l, edge_sel);
#endif
...
void gpio_set_data(u32 port, u32 sig_no, u32 data)
{
	...
	spin_lock_irqsave(&port_p->lock, flags);
	_set_gpio_dataout(port_p, sig_no, (data == 0) ? 0 : 1);
	spin_unlock_irqrestore(&port_p->lock, flags);
}
...
EXPORT_SYMBOL(mxc_get_gpio_direction);
