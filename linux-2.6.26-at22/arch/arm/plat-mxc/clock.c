...
struct clk *clk_get(struct device *dev, const char *id)
{
	...
	if (str) {
		...
		list_for_each_entry(p, &clocks, node) {
			if (p->id == idno &&
			    strncmp(id, p->name, strlen(p->name)) == 0 &&
	...
	list_for_each_entry(p, &clocks, node) {
		if (strncmp(id, p->name, strlen(p->name)) == 0 && try_module_get(p->owner)) {
			clk = p;
			goto found;
		}
	}
...
static int mxc_clocks_proc_show(struct seq_file *m, void *v)
{
	struct clk *clkp;

	list_for_each_entry(clkp, &clocks, node) {
		seq_printf(m, "%s-%d:\t\t%lu, %d",
...
static int mxc_clocks_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, mxc_clocks_proc_show, NULL);
}

static const struct file_operations mxc_clocks_proc_fops = {
	.owner= THIS_MODULE,
	.open= mxc_clocks_proc_open,
	.read= seq_read,
	.llseek= seq_lseek,
...
static int __init mxc_setup_proc_entry(void)
{
	...
	res = proc_create("cpu/clocks", S_IRUGO, NULL,
			  &mxc_clocks_proc_fops);
