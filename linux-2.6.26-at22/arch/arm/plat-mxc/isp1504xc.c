...
static int isp1504_scratch_test(u32 *view)
{
	int timeout;
	...
	if (!(__raw_readl(view) & ULPIVW_SS)) {
		...
		timeout = 0x100000;
		while (timeout--) 
			if (!(__raw_readl(view) & ULPIVW_WU))
				break;
		if (timeout < 0) {
			pr_debug("isp1504 scratch test faild: "
				 "can not wake up.\n");
			return -1;
		}
	}
	...
	__raw_writel((ULPIVW_RUN | ULPIVW_WRITE |
		      (ISP1504_SCRATCH << ULPIVW_ADDR_SHIFT) |
		      ((0x5a & ULPIVW_WDATA_MASK) << ULPIVW_WDATA_SHIFT)),
		     view);

	timeout = 0x100000;
	while (timeout--)
		if (!(__raw_readl(view) & ULPIVW_RUN))
			break;
	if (timeout < 0) {
		pr_debug("isp1504 scratch test faild: not completed.\n");
		return -1;
	}
 ...
 * set vbus power
 *
 * @param       ops   pointer of struct fsl_xcvr_ops
 * @param       pdate pointer of struct fsl_usb2_platform_data
 * @param       on    power on or off
 */
static void isp1504_set_vbus_power(struct fsl_xcvr_ops *ops,
...
static void isp1504_reset(u32 *view)
{
	u8 data;

	isp1504_set(0x20, ISP1504_FUNCCTL, view);
	do {
		data = isp1504_read(ISP1504_FUNCCTL, view);
	} while (data & 0x20);
}
...
static struct fsl_xcvr_ops isp1504_ops = {
	...
	.scratch_test = isp1504_scratch_test,
	.reset = isp1504_reset,
};
