...
static struct clk per_clk[] = {
	 ...
	 .flags = RATE_PROPAGATES,},
	{
	 .name = "per_ssi1_clk",
	 .id = 13,
	 .parent = &upll_24610k_clk,	/* can be AHB or UPLL or 24.61MHz */
	 .round_rate = _clk_perclkx_round_rate,
	 .set_rate = _clk_perclkx_set_rate,
	 .set_parent = _clk_perclkx_set_parent3,
	 .recalc = _clk_perclkx_recalc,
...
unsigned long __init clk_early_get_timer_rate(void)
{
	...
	clk_set_parent(&per_clk[5], &ahb_clk);
	clk_set_rate(&per_clk[5], ahb_clk.rate);
...
int __init mxc_clocks_init(void)
{
	...
	/* the SSI clock must set to 24.6MHz  clock */
	clk_set_parent(&per_clk[13], &upll_24610k_clk);
	clk_set_rate(&per_clk[13], 24610000);
	clk_set_parent(&per_clk[14], &upll_24610k_clk);
	clk_set_rate(&per_clk[14], 24610000);

	/* the eSDHC clock must be derived from upll 240MHz clock */
	clk_set_parent(&per_clk[3], &upll_clk);
	clk_set_rate(&per_clk[3], 240000000);
	clk_set_parent(&per_clk[4], &upll_clk);
	clk_set_rate(&per_clk[4], 240000000);
